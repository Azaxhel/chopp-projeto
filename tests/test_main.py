...
def test_calculate_report_metrics_com_dados():
    vendas_exemplo = [
        Venda(data=date(2025, 10, 1), total=100.0, cartao=100.0, dinheiro=0.0, pix=0.0, custo_func=10.0, custo_copos=5.0, custo_boleto=0.0, lucro=85.0, dia_semana="", tipo_venda=""),
        Venda(data=date(2025, 10, 2), total=250.50, cartao=250.50, dinheiro=0.0, pix=0.0, custo_func=10.0, custo_copos=12.5, custo_boleto=2.0, lucro=226.0, dia_semana="", tipo_venda=""),
        Venda(data=date(2025, 10, 3), total=50.0, cartao=50.0, dinheiro=0.0, pix=0.0, custo_func=10.0, custo_copos=2.5, custo_boleto=0.0, lucro=37.5, dia_semana="", tipo_venda=""),
    ]
...
def test_criar_e_listar_produtos():
    ...
    client.post("/produtos", data={"nome": "Chopp IPA", "preco_venda_barril_fechado": 750.0})
...
def test_registrar_venda_feira():
    ...
    response_produto = client.post("/produtos", data={"nome": "Pilsen", "preco_venda_barril_fechado": 600.0, "volume_litros": 50, "preco_venda_litro": 20.0})
    assert response_produto.status_code == 200

    venda_data = {
        "data": "2025-10-10", "produto_id": 1, "tipo_venda": "feira",
        "total": 500.0, "cartao": 500.0, "dinheiro": 0.0, "pix": 0.0,
        "custo_func": 50.0, "custo_copos": 25.0, "custo_boleto": 0.0
    }
    ...
    with Session(engine) as session:
        venda_registrada = session.exec(select(Venda).where(Venda.data == date(2025, 10, 10))).first()
        ...
        movimento_estoque = session.exec(select(MovimentoEstoque).where(MovimentoEstoque.tipo_movimento == "saida_venda")).first()
...
def test_registrar_venda_barril_festas():
    ...
    client.post("/produtos", data={"nome": "IPA", "preco_venda_barril_fechado": 750.0, "volume_litros": 50, "preco_venda_litro": 22.0})
    client.post("/estoque/entrada", data={"produto_id": 1, "quantidade": 10, "custo_unitario": 400.0, "data_movimento": "2025-10-01"})

    venda_data = {
        "data": "2025-10-11", "produto_id": 1, "tipo_venda": "barril_festas",
        "quantidade_barris_vendidos": 2, "cartao": 1500.0, "dinheiro": 0.0, "pix": 0.0
    }
    response = client.post("/registrar_venda", data=venda_data)
    assert response.status_code == 200
    ...
    # Agora, verifica diretamente no banco de dados
    with Session(engine) as session:
        venda_registrada = session.exec(select(Venda).where(Venda.data == date(2025, 10, 11))).first()
        assert venda_registrada is not None
        ...
        movimento_estoque = session.exec(select(MovimentoEstoque).where(MovimentoEstoque.tipo_movimento == "saida_venda_barril")).first()
        assert movimento_estoque is not None
